...
with an MCP server that exposes tools (e.g., a memory server).

The main part of the script demonstrates connecting to the memory server 
(localhost:8030) and calling available tools.
"""
import os
import asyncio
...
async def main():
    """Main function to demonstrate MCPClient usage with the Memory Server."""
    # Hardcoded values for Memory Server
    target_server_url = "http://localhost:8030/sse"
    target_api_key = "secret-key3"
    print(f"SimpleClient: Targeting Memory Server at {target_server_url}, API_Key_Ending_With={target_api_key[-4:]}")
        ...
        for tool in client.tools:
            ...
            try:
                if tool_name == 'search_memories':
                    response = await client.call_tool(tool_name=tool_name, tool_input={"query": "Recent memories"})
                elif tool_name == 'save_memory':
                    response = await client.call_tool(tool_name=tool_name, tool_input={"content": "Test memory data"})
                else:
