...
with an MCP server that exposes tools (e.g., a crawler server).

The main part of the script demonstrates connecting to the crawler server 
(localhost:3006) and calling available tools.
"""
import os
import asyncio
...
async def main():
    """Main function to demonstrate MCPClient usage with the Crawler Server."""
    # Hardcoded values for Crawler Server
    target_server_url = "http://localhost:3006/sse"  # Updated port from 3005 to 3006
    target_api_key = "secret-key4"
    print(f"SimpleClient: Targeting Crawler Server at {target_server_url}, API_Key_Ending_With={target_api_key[-4:]}")
        ...
        for tool in client.tools:
            ...
            try:
                if tool_name == 'crawl_page':
                    response = await client.call_tool(tool_name=tool_name, tool_input={"url": "https://example.com"})
                elif tool_name == 'crawl_site':
                    response = await client.call_tool(tool_name=tool_name, tool_input={"url": "https://example.com", "max_depth": 1})
                elif tool_name == 'search_page':
                    response = await client.call_tool(tool_name=tool_name, tool_input={"url": "https://example.com", "query": "example"})
                else:
